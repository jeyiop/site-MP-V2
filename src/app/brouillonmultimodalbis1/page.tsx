...
export default function BrouillonMultimodalBis1() {
      ...
      <section style={{ backgroundColor: '#000B58' }} className="pt-3">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 border-b border-white/10">
          {CATEGORIES.map((cat, i) => {
            const Icon = cat.icon;
            return (
              <div key={cat.id} className={`flex flex-col items-center gap-2 py-4 px-4 text-center ${i < 3 ? 'border-r border-white/10' : ''}`}>
                {/* Picto dans cercle */}
                <span className="flex h-11 w-11 items-center justify-center rounded-full bg-white/12 border border-white/20 mb-1">
                  <Icon className="w-5 h-5 text-white" />
                </span>
                {/* Titre */}
                <span className="text-sm font-bold text-white uppercase tracking-[0.18em]">{cat.title}</span>
                {/* Ligne décorative */}
