...
export default function Brouillon() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-white">
      ...
      <section className="relative mt-[5vh] mb-[5vh] h-[57vh] min-h-[308px] w-full overflow-hidden bg-white">
      ...
      {/* Services Section — barre de catégories */}
      <section style={{ backgroundColor: '#000B58' }} className="py-10">
        <div className="container mx-auto px-4">
          {solutionsLoading ? (
            <div className="grid grid-cols-2 md:grid-cols-4 gap-0">
              {Array.from({ length: 4 }).map((_, index) => (
                <div key={index} className="flex flex-col items-center gap-3 px-6 py-7 animate-pulse">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 divide-x divide-white/15">
              {solutionOrder.map((orderIdx, displayIdx) => {
                const solution = displayedSolutions[orderIdx];
                const Icon = solution.icon;
                return (
                  <div key={solution.id} className="relative">
                    {editorMode && (
                      <div className="absolute -top-3 left-1/2 -translate-x-1/2 z-50 flex items-center gap-1 bg-white rounded-full px-2 py-1 shadow-lg">
                        <span className="text-[#000B58] text-[10px] font-bold uppercase tracking-wider">{displayIdx + 1}</span>
                        <button onClick={() => moveBlock(displayIdx, 'up')} disabled={displayIdx === 0} className="text-[#000B58] disabled:opacity-30 px-1 text-xs font-bold">◀</button>
                        <button onClick={() => moveBlock(displayIdx, 'down')} disabled={displayIdx === solutionOrder.length - 1} className="text-[#000B58] disabled:opacity-30 px-1 text-xs font-bold">▶</button>
                      </div>
                    )}
                    <Link
                      href={solution.link}
                      className={`group flex flex-col items-center gap-3 px-6 py-8 text-center transition-colors duration-200 hover:bg-white/10 rounded-none ${editorMode ? 'ring-2 ring-inset ring-white/30' : ''}`}
                    >
                      <span className="flex h-12 w-12 items-center justify-center rounded-xl bg-white/15 group-hover:bg-white/25 transition-colors duration-200">
                        <Icon className="w-7 h-7 text-white" />
                      </span>
                      <span className="text-base font-bold text-white">{solution.title}</span>
                      <span className="text-sm leading-snug text-white/60 max-w-[180px]">{solution.description}</span>
                    </Link>
                  </div>
                );
              })}
